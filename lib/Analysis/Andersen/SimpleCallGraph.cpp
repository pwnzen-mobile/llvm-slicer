...
SimpleCallGraph::FunctionSet_t &
SimpleCallGraph::getCalled(const Instruction *Inst) {
...
void SimpleCallGraph::finalize() {
  ...
  Function *ExternalFunc = dyn_cast<Function>(
      M.getOrInsertFunction("_EXTERNAL_", AttributeSet(),
                            Type::getVoidTy(getGlobalContext()), nullptr));
  assert(ExternalFunc);
  BasicBlock *BB =
      BasicBlock::Create(getGlobalContext(), "entry", ExternalFunc);
  ...
  for (FunctionSet_t::iterator F_it = External.begin(); F_it != External.end();
       ++F_it) {
...
void SimpleCallGraph::print(raw_ostream &ostream) {
  ...
  for (CallGraph_t::iterator C_it = CallGraph.begin(); C_it != CallGraph.end();
       ++C_it) {
    //        if
    //        (!(C_it->first->getParent()->getParent()->getName().startswith("-")
    //        ||
    //        C_it->first->getParent()->getParent()->getName().startswith("+")))
    //            continue;
    for (FunctionSet_t::iterator F_it = C_it->second->begin();
         F_it != C_it->second->end(); ++F_it) {
      //            if (!((*F_it)[0] == '+' || (*F_it)[0] == '-'))
      //                continue;
      if ((*F_it).find("objc") == 0) {
        continue;
      ...
      //            ostream << "\"" <<
      //            C_it->first->getParent()->getParent()->getName() << "\" ->
      //            \"" << *F_it << "\";\n";

      Edges.insert("\"" +
                   C_it->first->getParent()->getParent()->getName().str() +
                   "\" -> \"" + *F_it + "\";\n");
    }
  }

  for (std::set<std::string>::iterator E_it = Edges.begin();
       E_it != Edges.end(); ++E_it) {
    ostream << *E_it;
  }
...
SimpleCallGraph::FunctionSet_t
SimpleCallGraph::getCalled(const std::string &fun) {
