...
cl::opt<bool> FastEnum("fast-enum",
                       cl::desc("Handles Objective-C fast enum functions"),
...
bool CallHandlerBase::isSelf(Instruction *v, Andersen *andersen) {
  ...
  StackAccessPass::OffsetMap_t &Offsets =
      SAP->getOffsets(v->getParent()->getParent());
  ...
  for (auto &O_it : Offsets) {
    ...
    for (auto &U_it : O_it.first->uses()) {
      if (const Instruction *ItoP =
              dyn_cast<const IntToPtrInst>(U_it.getUser())) {
        ...
        if (const Instruction *ItoP =
          ...
          for (auto &IU_it : ItoP->uses()) {
            if (const LoadInst *Load =
                    dyn_cast<const LoadInst>(IU_it.getUser())) {
              ...
              if (Load == v) {
                ...
                for (auto &LU_it : Load->uses()) {
                  if (const StoreInst *Store =
                          dyn_cast<const StoreInst>(LU_it.getUser())) {
                    if (const IntToPtrInst *StoreItoP =
                            dyn_cast<IntToPtrInst>(Store->getOperand(1))) {
                      if (SAP->getOffsets(f)[StoreItoP->getOperand(0)]) {
                        StackAccessPass::Int64List_t StoreOffsets =
                            *SAP->getOffsets(f)[StoreItoP->getOperand(0)];
                        StoredAtOffsets.insert(StoredAtOffsets.end(),
                                               StoreOffsets.begin(),
                                               StoreOffsets.end());
                      }
                    }
  ...
  for (std::vector<const Value *>::iterator Self_it = SelfPtsTo.begin();
       Self_it != SelfPtsTo.end(); ++Self_it) {
    for (std::vector<const Value *>::iterator X0_it = X0PtsTo.begin();
         X0_it != X0PtsTo.end(); ++X0_it) {
      if (*X0_it == *Self_it)
        return true;
    }
  }
...
bool objcMsgSend::run(const Instruction *CallInst, std::string &F,
  ...
  for (DetectParametersPass::UserSet_t::iterator X0_it = X0Values.begin();
    ...
    User *X0 = *X0_it;
    for (DetectParametersPass::UserSet_t::iterator X1_it = X1Values.begin();
         X1_it != X1Values.end(); ++X1_it) {
      User *X1 = *X1_it;
...
//        instructionOffsetPrinter(dyn_cast<const Instruction>(X1Inst));
      ...
      for (std::vector<const Value *>::iterator X0PT_it = X0PT.begin();
           X0PT_it != X0PT.end(); ++X0PT_it) {

        auto handleLambda = [&](StringRef Classname, bool Meta) {
          for (std::vector<const Value *>::iterator X1PT_it = X1PT.begin();
               X1PT_it != X1PT.end(); ++X1PT_it) {
        ...
        bool Meta = false;
        if (ConstantDataArray *ClassData =
                dyn_cast<ConstantDataArray>((Value *)*X0PT_it)) {
      ...
      if (isObjectiveCMethod(CallInst->getParent()->getParent()->getName()) &&
          isSelf(dyn_cast<Instruction>(X0), andersen)) {
        if (X1PT.size()) {
          StringRef ClassName =
              getClassname(CallInst->getParent()->getParent()->getName());
          bool Meta =
      ...
      if (X0Inst && X0Inst->getOpcode() == Instruction::Load) {
        ...
        if (PatternMatch::match(
                X0Inst->getOperand(0),
                PatternMatch::m_IntToPtr(
                    PatternMatch::m_ConstantInt(ClassAddress)))) {
      ...
      if (ClassMethod && ClassName.size()) {
        if (SelectorName == "alloc" || SelectorName == "new" ||
          ...
          if (!andersen->getCallGraph().containtsEdge(CallInst, AllocMethod)) {
            ...
            for (DetectParametersPass::UserSet_t::iterator X0Post_it =
                     X0Post.begin();
              ...
              if (valIndex == AndersNodeFactory::InvalidIndex)
                valIndex =
                    andersen->getNodeFactory().createValueNode(*X0Post_it);
              NodeIndex objIndex =
                  andersen->getNodeFactory().getObjectNodeFor(*X0Post_it);
              if (objIndex == AndersNodeFactory::InvalidIndex)
                objIndex =
                    andersen->getNodeFactory().createObjectNode(*X0Post_it);
              assert(valIndex != AndersNodeFactory::InvalidIndex &&
                     objIndex != AndersNodeFactory::InvalidIndex);
              //                            andersen->addConstraint(AndersConstraint::ADDR_OF,
              //                            valIndex, objIndex);
              andersen->setType(*X0Post_it, ClassName);
              andersen->addConstraint(AndersConstraint::ADDR_OF, valIndex,
                                      objIndex);
            }
          }
        }
        ...
        for (unsigned i = 0; i < PtsTo.size(); ++i) {
          ...
          for (auto &type_it : C) {
            std::deque<std::string> Candidates =
                andersen->getMachO().getMethodCandidates(type_it, SelectorName,
                                                         ClassMethod);
            bool h = false;
            for (std::deque<std::string>::iterator C_it = Candidates.begin();
                 C_it != Candidates.end(); ++C_it) {
              if (Function *F = CallInst->getParent()
                                    ->getParent()
                                    ->getParent()
                                    ->getFunction(*C_it)) {
...
void objcMsgSend::handleCall(StringRef ClassName, StringRef MethodName,
                             bool Meta, Instruction *CallInst, Value *X0,
                             Value *X1, Andersen *andersen) {
  ...
  std::deque<std::string> Candidates = andersen->getMachO().getMethodCandidates(
      ClassName.str(), MethodName.str(), Meta);
...
bool objcARC::run(const Instruction *CallInst, std::string &F,
                  Andersen *andersen) {
  if (andersen->getCallGraph().containtsEdge(CallInst, F))
    return true;
  DetectParametersPass::UserSet_t X0Values =
...
bool dispatchBlock::run(const Instruction *CallInst, std::string &F,
                        Andersen *andersen) {
  ...
  if (F == "dispatch_async" || F == "dispatch_sync" || F == "dispatch_once") {
    blockReg = 6;
  } else if (F == "dispatch_after" ||
             F == "-[NSOperationQueue addOperationWithBlock:]") {
    blockReg = 7;
  }
  assert(blockReg);

  DetectParametersPass::UserSet_t Block =
      DetectParametersPass::getRegisterValuesBeforeCall(blockReg, CallInst);
  for (DetectParametersPass::UserSet_t::iterator Block_it = Block.begin();
       Block_it != Block.end(); ++Block_it) {
    ...
    for (std::vector<const Value *>::iterator PtsTo_it = PtsTo.begin();
      ...
      if (const ConstantInt *constant = dyn_cast<const ConstantInt>(loc)) {
        ...
        uint64_t BlockAddress =
            andersen->getMachO().getRAWData<uint64_t>(BlockAddressLocation);
      ...
      Andersen::FunctionIntPairSet_t &fs =
          andersen->getStackOffsets()[*PtsTo_it];
      StackAccessPass &StackAccess = andersen->getAnalysis<StackAccessPass>();

      for (auto &fs_it : fs) {
        StackAccessPass::OffsetValueListMap_t &OffsetValues =
            StackAccess.getOffsetValues((Function *)fs_it.first);
        ...
        for (StackAccessPass::ValueList_t::iterator FAV_it =
          ...
          for (std::vector<const Value *>::iterator FAC_it =
            ...
            Andersen::FunctionIntPairSet_t blockStackOffsets =
                andersen->getStackOffsets()[*FAC_it];
            for (auto &blockOffset_it : blockStackOffsets) {
              if (!StackAccess.getOffsetValues(
                      (Function *)blockOffset_it.first)[blockOffset_it.second])
                continue;
              StackAccessPass::ValueList_t &Vals = *StackAccess.getOffsetValues(
                  (Function *)blockOffset_it.first)[blockOffset_it.second];
              for (auto &V_it : Vals) {
                for (Value::const_use_iterator ITPUse_it = V_it->use_begin();
                     ITPUse_it != V_it->use_end(); ++ITPUse_it) {
                  if (Instruction *I =
                          dyn_cast<Instruction>(ITPUse_it->getUser())) {
                    ...
                    for (Value::const_use_iterator STUse_it = I->use_begin();
                      ...
                      if (Instruction *I2 =
                        ...
                        if (ConstantInt *Address =
                          ...
                          std::string FName =
                              andersen->getMachO().getFunctionName(
                                  Address->getZExtValue());
                          //                                                    errs() << FName << "\n";
                          Function *F =
                              andersen->getModule().getFunction(FName);
                          if (andersen->getCallGraph().containtsEdge(CallInst,
                                                                     FName))
                            return true;
                          andersen->getCallGraph().addCallEdge(CallInst, FName);
                          handled = true;
                          ...
                          DetectParametersPass::ParameterAccessPairSet_t
                              StackParams =
                                  andersen->getAnalysis<DetectParametersPass>()
                          ...
                          for (DetectParametersPass::ParameterAccessPairSet_t::
                            ...
                            Instruction *address =
                                andersen->findSetStackParameterInstruction(
      ...
      Function *Func =
          (Function *)((Instruction *)(*Block_it))->getParent()->getParent();
                ...
                for (Value::const_use_iterator STUse_it = I->use_begin();
                  ...
                  if (Instruction *I2 =
                    ...
                    if (ConstantInt *Address =
                      ...
                      DetectParametersPass::ParameterAccessPairSet_t
                          StackParams =
                              andersen->getAnalysis<DetectParametersPass>(*F)
                                  .getStackParameters(*F, SPIdx);
...
bool retainBlock::run(const Instruction *CallInst, std::string &F,
                      Andersen *andersen) {
  andersen->addToWorklist((Instruction *)CallInst);
...
bool specialAllocs::shouldHandleCall(std::string &F) { return false; }
bool specialAllocs::run(const Instruction *CallInst, std::string &F,
                        Andersen *andersen) {
  return false;
}
...
bool ClassHandler::run(const Instruction *CallInst, std::string &F,
                       Andersen *andersen) {
  if (andersen->getCallGraph().containtsEdge(CallInst, F)) {
    return true;
  }
  ...
  DetectParametersPass::UserSet_t X0Post =
      DetectParametersPass::getRegisterValuesAfterCall(5, CallInst);
  ConstantDataArray *ClassName =
      (ConstantDataArray *)ConstantDataArray::getString(getGlobalContext(),
                                                        getClassname(F), false);
...
bool ExternalHandler::run(const Instruction *CallInst, std::string &F,
                          Andersen *andersen) {
  if (!andersen->getCallGraph().containtsEdge(CallInst, F)) {
    //                        errs() << AllocMethod << "\n";
...
bool MsgSendSuper::run(const Instruction *CallInst, std::string &F,
               ...
               */
                ...
                if (const StoreInst *store2 =
                  ...
                  if (const IntToPtrInst *ItoP2 =
                          dyn_cast<const IntToPtrInst>(store2->getOperand(1))) {
                    if (!andersen->getAnalysis<StackAccessPass>().getOffsets(
                            (Function *)ItoP->getParent()
                      ...
                      for (auto &super_it : Super) {
                        if (const ConstantDataArray *superDataArray =
                          ...
                          for (auto &preX1_it : PreX1) {
                            ...
                            for (auto &sel_it : Sel) {
                              if (const ConstantDataArray *selector =
                                      dyn_cast<const ConstantDataArray>(
                                          sel_it)) {
  ...
  for (auto &infos : calls) {
    bool HandledCall = false;
    std::deque<std::string> Candidates =
        andersen->getMachO().getMethodCandidates(
...
bool CopyProperty::run(const Instruction *CallInst, std::string &F,
                       Andersen *andersen) {
  andersen->addToWorklist((Instruction *)CallInst);
  ...
  DetectParametersPass::UserSet_t dst =
  ...
  for (auto &pre :
    ...
    for (auto &pts : prePts) {
      if (CallInst->getParent()->getParent()->getName() ==
          "-[FormViewController setCancelPressed:]") {
        assert(true);
      }
...
bool NSArray::run(const Instruction *CallInst, std::string &F,
  ...
  DetectParametersPass::UserSet_t objects =
      DetectParametersPass::getRegisterValuesBeforeCall(7, CallInst);
...
void NSArray::handleFastEnum(const Instruction *CallInst, Andersen *andersen) {
  DetectParametersPass::UserSet_t contextValues =
      DetectParametersPass::getRegisterValuesBeforeCall(7, CallInst);
  DetectParametersPass::UserSet_t arrayValues =
      DetectParametersPass::getRegisterValuesBeforeCall(5, CallInst);
        ...
        for (uint64_t stackOffset : *stackOffsets[contextValue]) {
          ...
          for (auto &offsetValue : *stackOffsetValues[objectsStart]) {
            for (auto offsetUser : offsetValue->users()) {
              const IntToPtrInst *offsetUserInst =
                  dyn_cast<const IntToPtrInst>(offsetUser);
              if (!offsetUserInst)
                continue;
              for (auto iToPUser : offsetUserInst->users()) {
                const LoadInst *iToPUserInst =
                    dyn_cast<const LoadInst>(iToPUser);
                if (!iToPUserInst)
                  continue;
                for (auto loadUser : iToPUser->users()) {
                  const AddOperator *loadUserInst =
                      dyn_cast<const AddOperator>(loadUser);
                  if (!loadUserInst)
                    continue;
                  ConstantInt *shl = nullptr;
                  if (!PatternMatch::match(
                          loadUserInst->getOperand(1),
                  ...
                  for (auto addUser : loadUserInst->users()) {
                    const IntToPtrInst *addUserInst =
                        dyn_cast<const IntToPtrInst>(addUser);
                    if (!addUserInst)
                      continue;
                    for (auto iToP2User : addUserInst->users()) {
                      const LoadInst *loadFromArray =
                          dyn_cast<const LoadInst>(iToP2User);
                      if (!loadFromArray)
                        continue;
                      NodeIndex loadValIdx =
                          andersen->getNodeFactory().getValueNodeFor(
                              loadFromArray);
                      if (loadValIdx == AndersNodeFactory::InvalidIndex)
                        loadValIdx = andersen->getNodeFactory().createValueNode(
                            loadFromArray);
                      andersen->addConstraint(AndersConstraint::LOAD,
                                              loadValIdx, arrayValIdx);
                    }
                  }
                }
              }
...
bool UIControlTarget::run(const Instruction *CallInst, std::string &F,
                          Andersen *andersen) {
  ...
  DetectParametersPass::UserSet_t controls =
      DetectParametersPass::getRegisterValuesBeforeCall(5, CallInst);
  DetectParametersPass::UserSet_t targets =
      DetectParametersPass::getRegisterValuesBeforeCall(7, CallInst);
  DetectParametersPass::UserSet_t selectors =
      DetectParametersPass::getRegisterValuesBeforeCall(8, CallInst);

  Andersen::StringSet_t controlTypes;

  auto setType = [andersen](const Function *fun, uint64_t regNo,
                            std::string type) {
    for (auto &inst : fun->getEntryBlock()) {
      ...
      const GetElementPtrInst *getElementPtrInst =
          dyn_cast<const GetElementPtrInst>(load->getOperand(0));
      ...
      for (auto &targetType : targetTypes) {
        for (auto &selector : selectors) {
          ...
          for (auto &selectorPtsTo : selectorPtsToSet) {
            const ConstantDataArray *selectorName =
                dyn_cast<const ConstantDataArray>(selectorPtsTo);
            if (!selectorName)
              continue;
...
bool UIAppDelegate::run(const Instruction *CallInst, std::string &F,
                        Andersen *andersen) {
  if (andersen->getCallGraph().containtsEdge(CallInst, F))
    return true;
  andersen->getCallGraph().addCallEdge(CallInst, F);

  DetectParametersPass::UserSet_t delegateRegs =
      DetectParametersPass::getRegisterValuesAfterCall(5, CallInst);
    ...
    for (auto &p : ((ObjectiveC::Class *)c.second.get())->getProtocolList()) {
      if (p == "UIApplicationDelegate") {
        ...
        for (auto delegateReg : delegateRegs) {
          NodeIndex valIdx =
              andersen->getNodeFactory().getValueNodeFor(delegateReg);
          if (valIdx == AndersNodeFactory::InvalidIndex)
            valIdx = andersen->getNodeFactory().createValueNode(delegateReg);
          NodeIndex objIdx =
              andersen->getNodeFactory().getObjectNodeFor(delegateReg);
          if (objIdx == AndersNodeFactory::InvalidIndex)
...
bool NSUserDefaults::run(const Instruction *CallInst, std::string &F,
                         Andersen *andersen) {
  if (andersen->getCallGraph().containtsEdge(CallInst, F)) {
    return true;
  }
  ...
  if (!singleton) {
    singleton = andersen->getNodeFactory().createDummy(
        *(Module *)CallInst->getParent()->getParent()->getParent());
    andersen->setType(singleton, "NSUserDefaults");
  }

  DetectParametersPass::UserSet_t returnRegs =
      DetectParametersPass::getRegisterValuesAfterCall(5, CallInst);
...
bool SecItemCopyAdd::run(const Instruction *CallInst, std::string &F,
                         Andersen *andersen) {
  if (andersen->getCallGraph().containtsEdge(CallInst, F))
    return true;
  andersen->addToWorklist((Instruction *)CallInst);
  ...
  for (auto &X1 : PreX1) {
    ...
    for (auto &pts1 : ptsToSet1) {
      ...
      andersen->getPointsToSet(andersen->getNodeFactory().getLocation(pts1),
                               ptsToSet2);

      if (ptsToSet2.size() == 0) {
        Value *dummy = andersen->getNodeFactory().createDummy(
            *(Module *)CallInst->getParent()->getParent()->getParent());
        NodeIndex objIdx = andersen->getNodeFactory().createObjectNode(dummy);
        NodeIndex valIdx = andersen->getNodeFactory().createValueNode(dummy);
        andersen->addConstraint(AndersConstraint::ADDR_OF, valIdx, objIdx);

        NodeIndex valIdx1 = andersen->getNodeFactory().getValueNodeFor(
            andersen->getNodeFactory().getLocation(pts1));
        if (valIdx1 == AndersNodeFactory::InvalidIndex) {
          valIdx1 = andersen->getNodeFactory().createValueNode(
              andersen->getNodeFactory().getLocation(pts1));
        }
...
bool NSDeclared::shouldHandleCall(std::string &F) {
  // We can add other declared NS functions here.
  if (F == "NSSearchPathForDirectoriesInDomains") {
    return true;
  }
  return false;
}

bool NSDeclared::run(const Instruction *CallInst, std::string &F,
                         Andersen *andersen) {
  #if 1
  if (andersen->getCallGraph().containtsEdge(CallInst, F))
    return true;
  ...
  DetectParametersPass::UserSet_t X1Values =
      DetectParametersPass::getRegisterValuesBeforeCall(6, CallInst);
  DetectParametersPass::UserSet_t X2Values =
      DetectParametersPass::getRegisterValuesBeforeCall(7, CallInst);
  DetectParametersPass::UserSet_t X0Post =
      DetectParametersPass::getRegisterValuesAfterCall(5, CallInst);
  ...
  for (DetectParametersPass::UserSet_t::iterator X0_it = X0Values.begin();
    ...
    for (DetectParametersPass::UserSet_t::iterator X0Post_it = X0Post.begin();
      ...
      if (dstIndex == AndersNodeFactory::InvalidIndex)
        dstIndex = andersen->getNodeFactory().createValueNode(*X0Post_it);
      andersen->addConstraint(AndersConstraint::ADDR_OF, dstIndex, srcIndex);
    }
  }
  #endif
  return true;
